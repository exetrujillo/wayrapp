...
describe('Card Component', () => {
  it('renders with default props', () => {
    ...
    expect(card).toHaveClass('card', 'rounded-component', 'bg-white', 'shadow-md');

    // Check content div has padding
    const contentDiv = screen.getByText('Card content').closest('div');
    expect(contentDiv).toHaveClass('p-6');
  });

  it('renders different variants correctly', () => {
    const { rerender } = render(<Card variant="default">Default</Card>);
    let card = screen.getByText('Default').closest('div')?.parentElement;
    expect(card).toHaveClass('bg-white', 'shadow-md');

    rerender(<Card variant="outlined">Outlined</Card>);
    card = screen.getByText('Outlined').closest('div')?.parentElement;
    expect(card).toHaveClass('bg-white', 'border', 'border-neutral-200');

    rerender(<Card variant="elevated">Elevated</Card>);
    card = screen.getByText('Elevated').closest('div')?.parentElement;
    expect(card).toHaveClass('bg-white', 'shadow-lg');
  });

  it('applies different padding options correctly', () => {
    const { rerender } = render(<Card padding="none">No padding</Card>);
    let contentDiv = screen.getByText('No padding').closest('div');
    expect(contentDiv).not.toHaveClass('p-3', 'p-6', 'p-8');

    rerender(<Card padding="sm">Small padding</Card>);
    contentDiv = screen.getByText('Small padding').closest('div');
    expect(contentDiv).toHaveClass('p-3');

    rerender(<Card padding="md">Medium padding</Card>);
    contentDiv = screen.getByText('Medium padding').closest('div');
    expect(contentDiv).toHaveClass('p-6');

    rerender(<Card padding="lg">Large padding</Card>);
    contentDiv = screen.getByText('Large padding').closest('div');
  ...
  it('renders with title only', () => {
    ...
    // Check for header border - need to find the header container div
    const header = screen.getByText('Card Title').closest('div')?.parentElement;
  ...
  it('renders with footer', () => {
    ...
    // Check for footer border - the footer container is the parent of the footer content
    const footerContainer = screen.getByTestId('card-footer').parentElement;
  ...
  it('does not add interactive styles when onClick is not provided', () => {
    ...
    const card = screen.getByText('Non-clickable card').closest('div')?.parentElement;
  ...
  it('maintains proper structure without header or footer', () => {
    ...
    const card = screen.getByText('Just content').closest('div')?.parentElement;
  ...
  it('handles keyboard interaction when clickable', () => {
    ...
    const card = screen.getByText('Keyboard accessible').closest('div')?.parentElement;
